...
<div class="mx-auto mt-5 mb-10">
    <form action="<?= base_url('indikatorKinerja/update_jurusan/' . $id) ?>" id="form-edit-indikator-jurusan" method="POST" show-validation>
        ...
        <div class="form-control">
            ...
            <textarea class="textarea textarea-bordered" name="keterangan" id="keterangan" placeholder="Masukkan Keterangan" required><?= $ik['keterangan'] ?></textarea>
        </div>
        ...
        <div class="form-control">
            ...
            <select type="text" name="jurusan[]" id="jurusan" multiple="multiple" data-placeholder="Pilih Jurusan" class="select select-bordered" style="width: 100%;">
                <?php foreach ($jurusan as $j) : ?>
                    <option value="<?= $j['jurusan_id'] ?>" <?= in_array($j['jurusan_id'], $ik['jurusan_id']) ? 'selected' : '' ?>><?= $j['nama_jurusan'] ?></option>
                <?php endforeach ?>
            </select>
        </div>
...
<script>
    ...
    $("#jurusan option:selected").each(function() {
        jurusanList.add({
            name: $(this).text(),
        })
            ...
            <tr>
                ...
                <td>
                    <div class="form-control text-center">
                        <input type="number" name="tw1[${jurusan_id}]" id="tw1" class="tw1 input input-bordered w-full" min="0" step="0.01" value="0" required>
                    </div>
                </td>
                <td>
                    <div class="form-control">
                        <input type="number" name="tw2[${jurusan_id}]" id="tw2"  class="tw2 input input-bordered w-full" min="0" step="0.01" value="0" required>
                    </div>
                </td>
                <td>
                    <div class="form-control">
                        <input type="number" name="tw3[${jurusan_id}]" id="tw3"  class="tw3 input input-bordered w-full" min="0" step="0.01" value="0" required>
                    </div>
                </td>
                <td>
                    <div class="form-control">
                        <input type="number" name="tw4[${jurusan_id}]" id="tw4"  class="tw4 input input-bordered w-full" min="0" step="0.01" value="0" required>
                    </div>
                </td>
            </tr>
        `,
                ...
                $(`#cascading${jurusan_id} option:selected`).each(function() {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            cascadingList.add({
                                name: cascading_name,
                                tw1: response.data.triwulan_satu,
                                tw2: response.data.triwulan_dua,
    ...
    initFormAjax('#form-edit-indikator-jurusan', {
        success: function(response) {
