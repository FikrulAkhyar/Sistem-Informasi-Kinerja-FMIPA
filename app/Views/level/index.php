...
<div class="lg:text-2xl text-xl font-bold">Kelola Level Pengguna</div>
<div class="mx-auto mt-5 mb-10">
    <div class="my-3">
        <div class="text-end my-3">
            <button class="btn btn-primary btn-tambah">Tambah Level</button>
        </div>
...
<script>
    const table = $('#table').DataTable({
        ...
        columns: [{
            ...
            {
                data: 'jumlah_pengguna',
                searchable: false,
                render: function(data) {
                    return `<span class="truncate overflow-ellipsis w-3/5 text-center"><p>${data}</p></span>`
                }
            },
            {
                data: 'level_id',
                ...
                render: function(data, _, row) {
                    let delete_btn = row.jumlah_pengguna == 0 ? `<button data-reference="${data}" class="btn btn-error btn-sm btn-delete-modal text-white" data-tippy-content="Hapus Level">
                    ...
                    return `
                       <div class="flex justify-center gap-2">
                            <button data-reference="${data}" class="btn btn-warning btn-sm btn-edit-modal text-white" data-tippy-content="Edit Level">
    ...
    $('.btn-tambah').on('click', function(e) {
        ...
        $.ajax({
            url: `${BASE_URL}/level/modal_create/`,
    ...
    $(document).on('click', '.btn-edit-modal', function() {
        ...
        $.ajax({
            url: `${BASE_URL}/level/modal_edit/${ref}`,
            success: function(response) {
                $('#modal-global-container.modal .modal-box').html(response.html)
                $('#modal-global').prop('checked', true)
            }
    ...
    $(document).on('click', '.btn-password-modal', function() {
        const ref = $(this).data('reference')
        $.ajax({
            url: `${BASE_URL}/level/modal_edit_password/${ref}`,
            success: function(response) {
                $('#modal-global-container.modal .modal-box').html(response.html)
                $('#modal-global').prop('checked', true)
            }
    ...
    $(document).on('click', '.btn-delete-modal', function() {
        const ref = $(this).data('reference')
        $.ajax({
            url: `${BASE_URL}/level/modal_delete/${ref}`,
            success: function(response) {
                $('#modal-global-container.modal .modal-box').html(response.html)
                $('#modal-global').prop('checked', true)
            }
    ...
    initFormAjax('#form-delete-level', {
        success: function(response) {
            $('#modal-global').prop('checked', false)
            toastr.success(response.message)
            table.draw()
    ...
    initFormAjax('#form-update-level', {
        success: function(response) {
            $('#modal-global').prop('checked', false)
            toastr.success(response.message)
            table.draw()
