...
<h2 class="font-bold text-xl lg:text-2xl my-4">Edit Level</h2>
<form action="<?= base_url('level/update/' . $id) ?>" id="form-update-level" method="POST" show-validation>
    <div class="form-control">
        <label for="nama_level" class="label">
            <span class="label-text label-required">Nama Level</span>
        </label>
        <input type="text" name="nama_level" id="nama_level" class="input input-bordered" placeholder="masukkan nama level" value="<?= $level['nama_level'] ?>">
    </div>
    <div class="form-control">
    ...
    <div class="modal-action">
        <button type="submit" form="form-update-level" class="btn btn-primary btn-block">Edit</button>
    </div>
</form>
